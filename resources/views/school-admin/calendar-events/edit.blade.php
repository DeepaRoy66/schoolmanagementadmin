<x-app-layout>
    ...
    @php
        $selectedType = old('type', $calendarEvent->type);
    @endphp
    ...
    <div class="p-6">
        ...
        <div class="bg-white rounded-lg border border-slate-200 shadow-sm p-6 max-w-xl">
            <form action="{{ route('school-admin.calendar-events.update', $calendarEvent) }}" method="POST">
                @csrf
                @method('PUT')

                <div class="mb-4">
                    <label for="title" class="block text-sm font-medium text-slate-700 mb-1">Title</label>
                    <input type="text" id="title" name="title" value="{{ old('title', $calendarEvent->title) }}"
                ...
                <div id="custom_type_wrapper" class="mb-4 {{ $selectedType === 'other' ? '' : 'hidden' }}">
                    ...
                    <input type="text" id="custom_type" name="custom_type"
                           value="{{ old('custom_type', $calendarEvent->custom_type) }}"
                ...
                <div class="mb-4 grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="date" id="start_date" name="start_date"
                               value="{{ old('start_date', $calendarEvent->start_date?->format('Y-m-d')) }}"
                               class="w-full text-sm border border-slate-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#1e4ed8]/30 focus:border-[#1e4ed8]"
                               required>
                        @error('start_date')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="date" id="end_date" name="end_date"
                               value="{{ old('end_date', $calendarEvent->end_date?->format('Y-m-d')) }}"
                               class="w-full text-sm border border-slate-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#1e4ed8]/30 focus:border-[#1e4ed8]">
                        @error('end_date')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="mb-4">
                    ...
                    <textarea id="description" name="description" rows="3"
                              class="w-full text-sm border border-slate-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#1e4ed8]/30 focus:border-[#1e4ed8]">{{ old('description', $calendarEvent->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6 flex items-center gap-2">
                    <input type="checkbox" id="is_recurring" name="is_recurring" value="1"
                           @checked(old('is_recurring', $calendarEvent->is_recurring))

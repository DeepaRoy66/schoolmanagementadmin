    ...
    <div class="py-8">
        <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="flex items-center gap-2">
                ...
                <span class="text-gray-500 text-sm">List of all fee names</span>
            </div>
            ...
            <div class="bg-white shadow rounded-md overflow-hidden border-t-4 border-indigo-600">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-6 py-5 border-b border-gray-200">
                    <form action="{{ route('school-admin.fee-names.index') }}" method="GET" class="flex items-center gap-2">
                ...
                <p class="px-6 pt-4 text-sm text-gray-500">
                    Total fee names: {{ $feeNames->total() }}
                </p>
                    ...
                    <table class="w-full text-sm text-left">
                        <thead>
                            <tr class="border-b border-gray-200 text-gray-600 bg-indigo-50/40">
                                ...
                                <th class="py-3 px-6 font-semibold">Fee Group</th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse ($feeNames as $fee)
                                ...
                                <tr class="hover:bg-gray-50">
                                    <td class="py-4 px-6 font-medium text-gray-900">{{ $fee->name }}</td>
                                    <td class="py-4 px-6 text-gray-500">{{ $fee->code }}</td>
                                    <td class="py-4 px-6 text-gray-700">{{ $fee->feeGroup->name ?? '—' }}</td>
                                    <td class="py-4 px-6 text-gray-700">{{ $typeLabels[$fee->fee_type] ?? $fee->fee_type }}</td>
                                    <td class="py-4 px-6">
                                        @if ($fee->is_active)
                                            <span class="px-2.5 py-1 bg-emerald-100 text-emerald-700 rounded-md text-xs font-semibold">Active</span>
                                        @else
                                            <span class="px-2.5 py-1 bg-gray-100 text-gray-500 rounded-md text-xs font-semibold">Inactive</span>
                                        @endif
                                    </td>
                                    <td class="py-4 px-6">
                                        <div class="flex items-center justify-end gap-2">
                                            <a href="{{ route('school-admin.fee-names.edit', $fee) }}"
                                            ...
                                            <form action="{{ route('school-admin.fee-names.destroy', $fee) }}" method="POST"
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="py-16 text-center text-gray-500 text-sm">
                                        @if (request('search'))
                                            No fee names found for "{{ request('search') }}".
                                        @else
                                            No fee names found.
                                            <a href="{{ route('school-admin.fee-names.create') }}" class="text-blue-600 hover:underline">Add a new fee name</a>.
